...
export default function WaitTimeScreen() {
  ...
  const { config, configLoading, setConfig, departures, loading, error, refresh } = useWaitTime();
    ...
    return (
      <View style={styles.screen}>
        <ScreenHeader
          ...
          leftAction={
            isConfiguring ? (
              <HeaderButton label="Annuler" onPress={() => setIsConfiguring(false)} />
            ) : (
              <HeaderButton label="‹ Retour" onPress={() => router.back()} variant="back" />
            )
          }
        />
  ...
  return (
    <View style={styles.screen}>
      <ScreenHeader
        ...
        leftAction={<HeaderButton label="‹ Retour" onPress={() => router.back()} variant="back" />}
        rightAction={
          <HeaderButton label="Modifier" onPress={() => setIsConfiguring(true)} />
        }
      />
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {departures.map((dep, i) => (
          <View
            ...
            style={[
              styles.card,
              i === 0 && { borderLeftWidth: 4, borderLeftColor: config.lineColor },
            ]}
          >
            <View style={styles.cardLeft}>
              ...
              {dep.tempsReel && (
                <View style={styles.realtimeBadge}>
                  <Text style={[styles.realtimeText, { color: config.lineColor }]}>
                    Temps réel
                  </Text>
                </View>
              )}
            ...
            <View style={styles.cardRight}>
              <Text
                style={[
                  styles.minutes,
                  i === 0 && { fontSize: 36, fontWeight: '700', color: config.lineColor },
                ]}
              >
